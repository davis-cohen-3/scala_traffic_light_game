...
//import edu.colorado.csci3155.project3.ShootDart.BaloonEscaped
...
case class Boom(x: Double, y: Double) extends Event {
    override def appliesTo(e: Entity): Boolean = e match {
        case ArcadeBaloon(_, x1, y1, _) => {
           val xSquare:Double = (x-x1)*(x-x1)
            val ySquare:Double = (y-y1)*(y-y1)
            val root:Double = xSquare+ySquare
            val rooted:Double = sqrt(root)
            if(rooted < ScreenDimensions.baloonBurstDistance) true
            else false /*-- TODO: Boom applies to an arcade baloon iff euclidean distance between (x1, y1) and (x,y) is less than ScreenDimensions.baloonBurstDistance --*/
        }
        case _ => false
    }
}
...
case class ArcadeGameManager(val gamePoints: Int) extends Entity {
    ...
    override def handleEvent(global: MainEventLoop)(e: Event): (List[Entity], List[Event]) = e match {
        case Tick(_) => {
            ...
            val rNum: Double = Random.nextDouble()
            if(rNum < .02){
                val randY:Double = 500+Random.nextInt(100)
                val randDrft: Double = 4+Random.nextInt(8)
                val newBal = new ArcadeBaloon(global.numEntities+1,20,randY,randDrft)
                val retList:(List[Entity], List[Event]) = (List(ArcadeGameManager(gamePoints),
                    newBal), List())
                retList
            }
            else if(rNum>.98){
                val randY:Double = 500+Random.nextInt(100)
                val randDrft: Double = -12+Random.nextInt(8)
                val newBal = new ArcadeBaloon(global.numEntities+1,20,randY,randDrft)
                val retList:(List[Entity], List[Event])= (List(ArcadeGameManager(gamePoints),
                    newBal),List())
                retList
                ...
                (List(ArcadeGameManager(gamePoints)),List())
            }
        }
        case BaloonEscaped =>  {
            (List(ArcadeGameManager(gamePoints+ScreenDimensions.pointsBaloonEscaped)),List())

        }
        case BaloonBurst => {
            (List(ArcadeGameManager(gamePoints+ScreenDimensions.pointsBaloonBurst)),List())
        }
        case Stop => (List(),List())
        case _ => (List(ArcadeGameManager(gamePoints)),List())
    }
...
case class ArcadeBaloon(id: Int, x: Double, y: Double, driftX: Double = 10.0 * Random.nextDouble) extends Entity {
    override def handleEvent(global: MainEventLoop)(e: Event): (List[Entity], List[Event]) =
        e match {
            case Tick(delta) => {
                val x1:Double = x+delta*driftX
                if(x1>=ScreenDimensions.shooterMax || x1<=0){
                    (List(),List(BaloonEscaped))
                }
                else{
                    (List(ArcadeBaloon(id,x1,y,driftX)),List())
                }
            }
            case Boom(_, _) => {
...
case class Dart(id: Int, x: Double, y: Double) extends Entity{
    override def handleEvent(global: MainEventLoop)(e: Event): (List[Entity], List[Event]) = e match  {
        case Tick(delta) => {
           val y1:Double = y-delta*ScreenDimensions.dartSpeed
            if(y1<=20){
                (List(),List())
            }
            else{
                (List(Dart(id,x,y1)),List(Boom(x,y1)))
            }
        }
        case Stop => (List(), List())
        case _ => (List(this), List())
