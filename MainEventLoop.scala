...
class MainEventLoop {
    ...
    def partition(l: List[Entity]) (ev: Event): (List[Entity], List[Entity]) = {
        val part_list = l.partition(e => ev.appliesTo(e))
        val l1 = part_list._1
        val l2 = part_list._2
        (l1, l2)
    }
    ...
    */
    def processOneEvent (l: List[Entity]) (ev: Event): (List[Entity], List[Event]) = {
        ...
        val (lApplies, lDoesNotApply) =  partition(l)(ev)
        val (l1: List[Entity], l2: List[Event]) = lApplies.foldLeft[(List[Entity],
          List[Event])]((List(), List()))((accList:(List[Entity], List[Event]), elem) =>
        {val e:(List[Entity], List[Event]) = elem.handleEvent(this)(ev)
            (accList._1++e._1, accList._2++e._2)
        })
        //foldLeft to apply the stuff in the for loop
        (lDoesNotApply ++ l1, l2)
   ...
   */
    final def performOneTransition: (List[Entity], List[Event]) = {
        /*-- TODO : implement performOneTransitionMutable without using for loops or var --*/
           val (newEntities: List[Entity], newEvents: List[Event], entities2: List[Entity]) =
               events.foldLeft((List[Entity](), List[Event](), entities))((accList:(List[Entity], List[Event], List[Entity]), elem) => {
                   val e:(List[Entity], List[Event]) = processOneEvent(accList._3)(elem)
               (e._1, accList._2++e._2, e._1)
           })
        (newEntities, newEvents)
    }
